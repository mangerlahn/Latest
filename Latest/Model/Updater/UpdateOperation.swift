...
class UpdateOperation: StatefulOperation, @unchecked Sendable {
	...
	override func finish() {
		let didCompleteSuccessfully = self.error == nil && !self.isCancelled && self.markCompletionNotificationPending()
		...
		if didCompleteSuccessfully {
			NotificationCenter.default.post(name: .latestUpdateOperationDidFinish, object: self, userInfo: [
